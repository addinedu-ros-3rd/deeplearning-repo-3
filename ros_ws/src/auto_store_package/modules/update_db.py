...
def log_checkIn_state(checkIn_time, customer_info):
    try:
        query = "insert into enterence (customer_ID, enterenceTime, enterStatus) values (%s, %s, %d)"
...
def log_checkOut_state(checkOut_time, customer_info):
    try:
        query = "insert into enterence (customer_ID, enterenceTime, totalAmount) values (%s, %s, %d)"
...
def make_payment(customer):
    ...
    try:
        query = "insert into payment (customer_ID, enterenceTime, totalAmount) values (%s, %s, %d)"
...
def log_action_state(action_time, action_type, fruit_id):
    try:
        query = "insert into actionRecognition (fruitID, actionTime, actionType, totalAmount) values (%d, %s, %s)"
...
#     try:
#         query = "insert into mismatchActionStand (standFruitID, actionFruitID, mismatch) values (%d, %d, %s)"
#         db.execute(query, (fruit_id, action_time, action_type))
#     except Exception as e:
#         log.error(f"make_payment() is error")
...
# 과일 table : 재고 update
def update_fruit_stock(fruit_type, fruit_values):
    try:
        query = "select stockStand from fruits where fruitID=%d"
        db.execute(query, fruit_type)

        fruit_counts = db.fetchone(query)

        query = "update fruits set stockStand=%d where fruitID=%s"
        db.execute(query, ((fruit_counts-fruit_values), fruit_type))
...
def get_price_list(fruit_type):
    try:
        query = "select price from fruits where fruitID=%d"
        db.execute(query, fruit_type)
